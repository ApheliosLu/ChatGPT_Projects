...
**注意：** 告诉你的人类伙伴，Superpowers 在有子代理支持时效果好得多（Codex、Codex CLI、Codex App、Copilot CLI 与 Gemini CLI 都算；见 `../using-superpowers/references/` 下的各平台工具参考）。如果子代理可用，请使用 superpowers:subagent-driven-development 而非此技能。
